...
// ValidateSplit checks that the `parts` and `threshold` are set to valid
// values. (modified from vault source to allow checking args prior to the
// entry of the secret)
func ValidateSplit(parts, threshold int) error {
	...
	if parts > 255 {
		return fmt.Errorf("parts cannot exceed 255")
	}
	if threshold < 2 {
		return fmt.Errorf("threshold must be at least 2")
	}
	if threshold > 255 {
		return fmt.Errorf("threshold cannot exceed 255")
	}
...
func Split(secret []byte, parts, threshold int) ([][]byte, error) {
	if err := ValidateSplit(parts, threshold); err != nil {
		return nil, err
	}
	if len(secret) == 0 {
